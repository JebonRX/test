...
wsstunnel="$(cat ~/log-install.txt | grep -w "Websocket HTTPS" | cut -d: -f2|sed 's/ //g')"
wsdropbear="$(cat ~/log-install.txt | grep -w "Websocket HTTP" | cut -d: -f2|sed 's/ //g')"
echo -e "\e[0;31m.-----------------------------------------.\e[0m"
echo -e "\e[0;31m|      \e[0;36mCHANGE PORT WEBSOCKET\e[m      \e[0;31m|\e[0m"
echo -e "\e[0;31m'-----------------------------------------'\e[0m"
echo -e " \e[1;31m>>\e[0m\e[1;33mChange Port For SSH & OVPN WS:\e[0m"
echo -e "     [1]  Change Port Websocket HTTP $wsdropbear"
echo -e "     [2]  Change Port Websocket HTTPS $wsstunnel"
echo -e "     [3]  Change Port Websocket XRAY "
echo -e "======================================"
...
;;
2)
read -p "New Port Websocket SSH(HTTPS) : " vpn
if [ -z $vpn ]; then
echo "Please Input Port"
exit 0
fi
